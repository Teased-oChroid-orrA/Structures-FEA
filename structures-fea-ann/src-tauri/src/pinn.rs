...
use crate::pino::{
    ...
    build_operator_prediction_with_params, canonical_backend_name, decode_prediction, enforce_training_recipe,
    default_holdout_validation,
    evaluate_holdout_projection_with_model, fit_operator_calibration, fit_operator_calibration_with_params,
    infer_config_for_case,
    is_pino_backend, model_config, operator_residual_score, runtime_metadata,
    spectral_probe_score, train_operator_calibration, OperatorCalibration,
    OperatorFieldHeadBatch, OperatorPrediction, OperatorTrainableParams, OperatorTrainingStats, PinoModelConfig,
    prediction_from_fem_with_grid, prediction_from_fem_with_grid_exact, PINO_BACKEND_CANDLE_CPU, PINO_BACKEND_CANDLE_CUDA,
...
use crate::pino_burn_head::{
    ...
    evaluate_operator_field_head_physics_grad_norm,
    train_operator_field_head_physics,
    BurnFieldHeadOptimizer,
    BurnPhysicsLossBreakdown, BurnPhysicsSample,
};
use rayon::prelude::*;
...
impl UniversalPinnEngine {
    ...
    fn build_burn_physics_sample_core(
        ...
        let target = prediction_from_fem_with_grid(
            &sample.case,
            &sample.fem,
            config.operator_grid.clone(),
        );
    ...
    fn build_burn_physics_sample_from_core(
        ...
        for idx in 0..cells {
            ...
            base_fields.extend_from_slice(&[
                ...
                batch.base_prediction.uz.get(idx).copied().unwrap_or(0.0)
                    * calibration.displacement_scale,
                batch.base_prediction.sxx.get(idx).copied().unwrap_or(0.0) * calibration.stress_scale,
                batch.base_prediction.syy.get(idx).copied().unwrap_or(0.0) * calibration.stress_scale,
                batch.base_prediction.szz.get(idx).copied().unwrap_or(0.0) * calibration.stress_scale,
                batch.base_prediction.sxy.get(idx).copied().unwrap_or(0.0) * calibration.stress_scale,
                batch.base_prediction.sxz.get(idx).copied().unwrap_or(0.0) * calibration.stress_scale,
                batch.base_prediction.syz.get(idx).copied().unwrap_or(0.0) * calibration.stress_scale,
            ...
            correction_scales.extend_from_slice(&[
                batch.correction_scales[scale_start] * calibration.displacement_scale,
                batch.correction_scales[scale_start + 1] * calibration.displacement_scale,
                batch.correction_scales[scale_start + 2] * calibration.displacement_scale,
                batch.correction_scales[scale_start + 3] * calibration.stress_scale,
                batch.correction_scales[scale_start + 4] * calibration.stress_scale,
                batch.correction_scales[scale_start + 5] * calibration.stress_scale,
                batch.correction_scales[scale_start + 6] * calibration.stress_scale,
                batch.correction_scales[scale_start + 7] * calibration.stress_scale,
                batch.correction_scales[scale_start + 8] * calibration.stress_scale,
                batch.correction_scales[scale_start + 9] * calibration.stress_scale,
                batch.correction_scales[scale_start + 10] * calibration.stress_scale,
            ]);
        }
        let (
            observable_target,
        ...
        let vm_residual = (0..cells)
            .map(|idx| {
                ...
                let base_vm = batch.base_prediction.von_mises.get(idx).copied().unwrap_or(0.0).abs()
                    * calibration.stress_scale;
        ...
        let mut ranked_hotspots = vm_residual
            ...
            .copied()
            .enumerate()
            .filter(|(idx, residual)| batch.mask.get(*idx).copied().unwrap_or(0.0) > 0.0 && *residual >= hotspot_threshold)
            .collect::<Vec<_>>();
        ranked_hotspots.sort_by(|a, b| {
            b.1.partial_cmp(&a.1)
        ...
        for (idx, residual) in ranked_hotspots.into_iter() {
            ...
            let separated = hotspot_centers.iter().all(|(cx, cy, cz, _): &(f64, f64, f64, f64)| {
                let dx = center.0 - *cx;
                let dy = center.1 - *cy;
                let dz = center.2 - *cz;
                ...
                for x in 0..config.operator_grid.nx {
                    ...
                    if hole_radius > 0.0 {
                        ...
                        let ring_narrow_band = (hole_radius * 0.18)
                            ...
                            .max(1e-6);
                        let ring_narrow =
                            (-((radial - hole_radius).powi(2)) / (ring_narrow_band * ring_narrow_band)).exp()
                                * angular_focus;
                        let center_x_focus = (-((px - hole_center_x).powi(2))
                            / (sample.case.geometry.length_in * 0.14).max(1e-6).powi(2))
                            .exp();
                        let thickness_focus = (-((pz - sample.case.geometry.thickness_in * 0.5).powi(2))
                        ...
                        let local_focus = ring_focus
                            ...
                            * thickness_focus;
                        let hole_focus_gain = env_f64("PINO_HOLE_STRESS_FOCUS_GAIN", 3.5).clamp(0.0, 8.0);
                    ...
                    if !hotspot_centers.is_empty() {
                        ...
                        for (cx, cy, cz, residual) in hotspot_centers.iter().copied() {
                            ...
                            let radius_z = 0.18;
                            let normalized =
                                (dx * dx + dy * dy) / (radius_xy * radius_xy) + (dz * dz) / (radius_z * radius_z);
                        ...
                        if patch_focus > 1.0e-6 {
                            ...
                            ring_loss_mask[idx] =
                                ring_loss_mask[idx].max((patch_ring_gain * patch_focus).clamp(0.0, 1.0));
                        }
                    }
                    ...
                    let axial_loaded =
                        sample.case.load.axial_load_lbf.abs() > 1e-9 && on_x_max;
                    let is_loaded =
                        !sample.case.boundary_conditions.fix_end_face && (vertical_loaded || axial_loaded);
                    ...
                    if is_loaded {
                        ...
                        if on_z_max {
                            set_normal((sample.case.geometry.thickness_in - pz).abs(), 0.0, 0.0, 1.0);
                        }
                        if hole_radius > 0.0 {
                            let dx = px - hole_center_x;
                            let dy = py - hole_center_y;
                            ...
                            if (radial - hole_radius).abs() <= hole_band && radial > 1e-9 {
                                set_normal((radial - hole_radius).abs(), dx / radial, dy / radial, 0.0);
                            }
                        }
                    }
        ...
        BurnPhysicsSample {
            ...
            dx: sample.case.geometry.length_in / (config.operator_grid.nx.saturating_sub(1).max(1) as f64),
            dy: sample.case.geometry.width_in / (config.operator_grid.ny.saturating_sub(1).max(1) as f64),
            dz: sample.case.geometry.thickness_in / (config.operator_grid.nz.saturating_sub(1).max(1) as f64),
    ...
    fn train_operator_field_head_burn_with_samples(
        ...
        if physics_samples.is_empty()
            || !current.matches_shape(PINO_FIELD_HEAD_BASIS, PINO_OUTPUT_CHANNELS)
        {
            return None;
        }
    ...
    fn direct_train_operator_params_for_targets(
        targets: &[OperatorFieldTarget],
        config: &PinoModelConfig,
        calibration: &OperatorCalibration,
        current: &OperatorTrainableParams,
    ...
    fn weighted_scale_mean(flat: &[f64], channel: usize, channels: usize, weights: &[f64]) -> f64 {
        weights
            .iter()
            .enumerate()
            .map(|(idx, weight)| flat.get(idx * channels + channel).copied().unwrap_or(0.0) * *weight)
            .sum::<f64>()
    }
    ...
    fn build_observable_bundle(
        ...
        let cells = target.grid.nx * target.grid.ny * target.grid.nz;
        ...
        let yz_center_sigma = (case.geometry.width_in.min(case.geometry.thickness_in) * 0.22).max(1e-6);

        for idx in 0..cells {
            let x_idx = idx % target.grid.nx;
            let yz_idx = idx / target.grid.nx;
            let y_idx = yz_idx % target.grid.ny;
            let z_idx = yz_idx / target.grid.ny;
            let xn = if target.grid.nx <= 1 {
                0.0
            } else {
                x_idx as f64 / (target.grid.nx - 1) as f64
            };
            let yn = if target.grid.ny <= 1 {
                0.0
            } else {
                y_idx as f64 / (target.grid.ny - 1) as f64
            };
            let zn = if target.grid.nz <= 1 {
                0.5
            } else {
                z_idx as f64 / (target.grid.nz - 1) as f64
            };
            ...
            let hole_ring = if hole_radius > 0.0 {
                ...
                let thickness_center = (-((z - case.geometry.thickness_in * 0.5).powi(2))
                    / (z_sigma * z_sigma))
                    .exp();
                ring
                    * (0.22 + 0.78 * angular_focus)
                    * (0.70 + 0.30 * x_center)
                    * thickness_center
            } else {
                0.0
            ...
            let loaded_top_corner = end_face
                * yn.powi(8)
                * (-((z - case.geometry.thickness_in * 0.5).powi(2)) / (yz_center_sigma * yz_center_sigma)).exp();
            let loaded_top_face = end_face
                * yn.powi(8)
                * (1.0 - (2.0 * zn - 1.0).abs().powi(2)).max(0.0);
            let root_top_face = (1.0 - xn).powi(8)
                * yn.powi(8)
                * (1.0 - (2.0 * zn - 1.0).abs().powi(2)).max(0.0);
            ...
            end_ux_projection[idx] = mask_weight * end_band.max(end_face * 0.65 * end_centerline.max(0.35));
            ...
            root_sigma_projection[idx] = mask_weight
                * (if hole_radius > 0.0 {
                    ...
                    let span_bending =
                        0.74 * cantilever_face_band + 0.16 * root_top_face + 0.10 * root_edge_scaled;
                    let span_blended =
                        (1.0 - cantilever_span_blend) * root_local + cantilever_span_blend * span_bending;
        ...
        let target_observable = vec![
            ...
            if hole_vm_observable {
                ...
                Self::weighted_channel_mean(
                    &target.sxx.iter().map(|value| value.abs()).collect::<Vec<_>>(),
                    &root_sigma_projection,
                )
            },
        ];
        let observable_scale = vec![
            Self::weighted_scale_mean(correction_scales, 1, PINO_OUTPUT_CHANNELS, &tip_uy_projection)
                .max(target_observable[0].abs() * 0.14)
                .max(Self::weighted_channel_mean(&base.uy, &tip_uy_projection).abs() * 0.12)
                .max(1e-6),
            Self::weighted_scale_mean(correction_scales, 0, PINO_OUTPUT_CHANNELS, &end_ux_projection)
                .max(target_observable[1].abs() * 0.12)
                .max(Self::weighted_channel_mean(&base.ux, &end_ux_projection).abs() * 0.10)
                .max(1e-6),
            Self::weighted_scale_mean(correction_scales, 9, PINO_OUTPUT_CHANNELS, &hotspot_vm_projection)
            ...
            .max(
                Self::weighted_channel_mean(&base.max_principal, &hotspot_principal_projection).abs() * 0.08,
            )
            .max(1e-3),
            if hole_vm_observable {
                Self::weighted_scale_mean(
                ...
                .max(
                    Self::weighted_channel_mean(&base.von_mises, &root_sigma_projection).abs() * 0.10,
                )
                .max(1e-3)
            } else {
                Self::weighted_scale_mean(
    ...
    fn build_operator_field_targets(
        ...
        targets
            ...
            .map(|case| {
                ...
                reduced.mesh.nz = reduced.mesh.nz.max(target_nz).clamp(2, 4);
    ...
    fn sample_operator_target_indices(
        total_targets: usize,
        sample_count: usize,
    ) -> Vec<usize> {
    ...
    fn curriculum_operator_grid(
        ...
        let mut grid = config.operator_grid.clone();
        let headless_fast_profile = low_target_mode && env_flag("PINO_HEADLESS_FAST_PROFILE", false);
        ...
        if factor < 0.999 {
            if grid.nx > 10 {
                grid.nx =
                    ((grid.nx as f64) * factor).round().clamp(10.0, grid.nx as f64) as usize;
            }
            if grid.ny > 8 {
                grid.ny = ((grid.ny as f64) * factor).round().clamp(8.0, grid.ny as f64) as usize;
            }
            if grid.nz > 2 {
                grid.nz = ((grid.nz as f64) * factor).round().clamp(2.0, grid.nz as f64) as usize;
            }
        }
        grid.nz = grid.nz.clamp(2, 4);
        grid
    ...
    fn evaluate_operator_epoch(
        ...
        let train_samples = targets
            .par_iter()
            .take(train_cut)
            .map(|sample| Self::build_burn_physics_sample(sample, &eval_config, calibration, params))
            .collect::<Vec<_>>();
            ...
            .map(|sample| {
                ...
                (
                    ...
                    Self::calibration_ratio(
                        ...
                        decoded.displacement_vector.get(1).copied().unwrap_or(0.0).abs(),
                    ),
                )
            })
            .collect::<Vec<_>>();
        ...
        C: FnMut(usize, UniversalPinnState, bool),
    {
        ...
        let emit_epoch_zero_progress = |on_epoch: &mut F,
                                        stage_id: &str,
                                        lr_phase: &str,
                                        architecture: &[usize],
                                        learning_rate: f64,
        ...
        let preflight_arch = pino_model_config
            ...
            .unwrap_or_else(|| {
                if status_before.architecture.is_empty() {
                    vec![crate::fem::ANN_INPUT_DIM, 12, 12, crate::fem::ANN_OUTPUT_DIM]
                } else {
        ...
        if let Some(cfg) = pino_model_config.clone() {
            emit_epoch_zero_progress(
                ...
                routed.learning_rate.unwrap_or(5e-4),
                Some(runtime_metadata(&routed, self.backend_runtime.tag(), cfg.spectral_modes)),
                0.0,
                0.0,
            );
            ...
            pino_model_config = Some(selected);
            if !fast_start_pino {
                pino_operator_training = stats;
            }
        }
        ...
        let active_param_config = pino_model_config
            .as_ref()
            .or(fallback_pino_config.as_ref());
        ...
        if self.backend_runtime != PinnBackendRuntime::CompatAnn {
            ...
            let pino_adapt_cooldown = (total_epochs / 32).clamp(16, 120);
            let pino_adapt_plateau = (total_epochs / 14).clamp(28, 120);
            let pino_adapt_overfit = (pino_adapt_plateau / 2).max(20);
            ...
            let startup_target_limit = if fast_start_pino {
                routed.cases.len().clamp(1, 3)
            } else {
                routed.cases.len()
            };
            let startup_target_cases = &routed.cases[..startup_target_limit];
            ...
            let foundation_note = format!(
                "PINOFoundation: backend={}, driver=operator-field-burn-only, initLR={:.6e}",
                self.backend_runtime.tag(),
                lr
            );
            let skip_exact_refine = matches!(
                std::env::var("PINO_HEADLESS_SKIP_EXACT_REFINE").ok().as_deref(),
                Some("1" | "true" | "TRUE" | "on" | "ON")
            ) || matches!(
            ...
            if let Some(cfg) = pino_model_config.as_ref() {
                ...
                stage_cfg.operator_grid =
                    Self::curriculum_operator_grid(cfg, 1, low_target_mode);
                emit_epoch_zero_progress(
                    &mut on_epoch,
                    "preflight",
                ...
                operator_targets = if operator_targets_expanded {
                    Self::build_operator_field_targets(startup_target_cases, stage_cfg.operator_grid)
                } else {
            ...
            let field_head_start_epoch = if low_target_mode {
                s1e.saturating_add(s2e).saturating_add(2).clamp(2, total_epochs.max(1))
            ...
            for epoch in 1..=total_epochs {
                ...
                let effective_stage_index =
                    if low_target_mode && headless_fast_profile && stage_index >= 3 && runtime_guard_low_budget
                    {
                ...
                train_cfg.operator_grid =
                    Self::curriculum_operator_grid(&active_cfg, effective_stage_index, low_target_mode);
                ...
                if !operator_targets_expanded && should_expand_operator_targets {
                        ...
                        .unwrap_or(false)
                    {
                        if let Some(staged) = staged_stage3_targets.take() {
                            ...
                            (
                                staged,
                                staged_stage3_target_cores.take(),
                            )
                        } else {
                            (
                    ...
                    if should_ramp_stage3_targets {
                        ...
                        let transition_indices =
                            Self::sample_operator_target_indices(full_targets.len(), transition_count);
                ...
                let stage_grid_changed = operator_targets_expanded && operator_targets
                    .first()
                    .map(|sample| sample.target.grid != train_cfg.operator_grid)
                    .unwrap_or(true);
                if stage_grid_changed {
                    operator_targets =
                        Self::build_operator_field_targets(&routed.cases, train_cfg.operator_grid.clone());
                ...
                let catastrophic_val = !val_loss.is_finite()
                    || (!best_val.is_finite() && !loss.is_finite())
                    || (best_val.is_finite() && val_loss > (best_val * 6.0).max(0.75))
                    || (!loss.is_finite())
                    || (best_val.is_finite() && loss > (best_val * 6.0).max(0.75));
                if catastrophic_val && best_val.is_finite() {
                    ...
                    stage_notes.push(format!(
                        ...
                        epoch, best_val, val_loss, rollback_count
                    ));
                    continue;
                }
                final_loss = loss;
                bootstrap_loss = loss;
                bootstrap_val_loss = val_loss;
                ...
                let field_head_stride = std::env::var("PINO_FIELD_HEAD_STRIDE")
                    ...
                    .unwrap_or_else(|| {
                        if low_target_mode {
                            if headless_fast_profile { 12 } else { 6 }
                        } else {
                            1
                        }
                    })
                ...
                if field_head_due {
                    ...
                    let head_optimizer = if optimizer_id == "pino-lbfgs"
                        || first_low_target_stage3_correction
                    {
                    ...
                    let head_steps = if head_optimizer == BurnFieldHeadOptimizer::Lbfgs {
                        if first_low_target_stage3_correction {
                            if headless_fast_profile { 14 } else { 18 }
                ...
                let holdout_refresh_stride = std::env::var("PINO_HOLDOUT_REFRESH_STRIDE")
                    ...
                    .unwrap_or_else(|| {
                        if low_target_mode {
                            if headless_fast_profile { 16 } else { 8 }
                        } else {
                            4
                        }
                    })
                ...
                let significant_improvement = best_val < f64::MAX
                    && best_val.is_finite()
                    && val_loss + (min_improvement * 8.0).max(best_val * 0.02) < best_val;
                let refresh_holdout = !disable_inner_holdout_refresh
                    && (epoch == 1
                        || epoch == total_epochs
                        || epoch % holdout_refresh_stride == 0
                        || significant_improvement);
                let selection_score = if refresh_holdout {
                    if let Some(cfg) = pino_model_config.as_ref() {
                    ...
                    let hotspot_score =
                        Self::hotspot_selection_score(holdout_summary.as_ref(), val_breakdown, val_loss);
                ...
                let selection_margin = if best_selection_score.is_finite() {
                    (best_selection_score.abs() * 0.002).max(min_improvement).max(1e-6)
                } else {
                ...
                let adaptation_ready = stage_index == 3
                    && epoch >= lbfgs_start_epoch
                    && rollback_count == 0
                    && best_val.is_finite()
                    && best_val <= 0.35
                    && trend_variance <= 1e-3;
                ...
                if can_attempt_pino_adapt && (plateau_trigger || overfit_trigger) {
                    ...
                    if let Some(active_cfg) = pino_model_config.clone() {
                        ...
                        if candidate != active_cfg {
                            ...
                            let candidate_params = OperatorTrainableParams::default()
                                .aligned_to_config(&candidate);
                ...
                let exact_refine_handoff_ready = focused_low_target_family
                    && !skip_exact_refine
                    && stage_index == 3
                    && epoch >= field_head_start_epoch.saturating_add(if headless_fast_profile {
                        4
                    } else {
                        8
                    })
            ...
            if low_target_mode
                ...
                && pino_model_config.is_some()
                && pino_calibration.is_some()
                && !skip_exact_refine
            {
                let cfg = pino_model_config.as_ref().expect("config checked above");
                let calibration = pino_calibration.as_ref().expect("calibration checked above");
                let headless_fast_profile = env_flag("PINO_HEADLESS_FAST_PROFILE", false);
                let exact_eval_weights = adaptive_weights;
                let exact_targets =
                    Self::build_operator_field_targets_exact(&routed.cases, cfg.operator_grid.clone());
                if !exact_targets.is_empty() {
                    let exact_samples = exact_targets
                        .iter()
                        .map(|sample| {
                    ...
                    let mut refinement_best_score = best_selection_score;
                    ...
                    if let Some(grad_norm) = evaluate_operator_field_head_physics_grad_norm(
                        ...
                        stage_notes.push(format!(
                            "PINOExactRefineProbe: cases={}, grid={}x{}x{}, gradNorm={:.6e}",
                            exact_samples.len(),
                    ...
                    stage_notes.push(format!(
                        "PINOExactRefineBaseline: exactLoss={:.6e}, score={:.6e}",
                    ...
                    let exact_refine_round_cap = std::env::var("PINO_HEADLESS_EXACT_REFINE_ROUNDS")
                        .ok()
                        .and_then(|raw| raw.parse::<usize>().ok())
                        .filter(|value| *value > 0)
                        .unwrap_or_else(|| if headless_fast_profile { 2 } else { usize::MAX });
                    let exact_refine_step_cap = std::env::var("PINO_HEADLESS_EXACT_REFINE_STEP_CAP")
                        .ok()
                        .and_then(|raw| raw.parse::<usize>().ok())
                        .filter(|value| *value > 0)
                        .unwrap_or_else(|| if headless_fast_profile { 32 } else { usize::MAX });
                    let mut refine_candidates = if focused_plate_hole {
                        vec![
                            (
                                BurnFieldHeadOptimizer::Lbfgs,
                                32usize,
                    ...
                    let refinement_rounds = if focused_plate_hole { 3usize } else { 2usize }
                        .min(exact_refine_round_cap);
                    let holdout_regression_limit = if focused_plate_hole { 1.35 } else { 1.20 };

                    'refine: for round in 0..refinement_rounds {
                        let mut round_improved = false;
                        for (optimizer, steps, learning_rate) in refine_candidates.iter().copied() {
                            let current_refine_params = refinement_best_params.clone();
                            ...
                            if let Some((candidate_params, candidate_train_loss)) =
                                Self::direct_train_operator_params_for_targets(
                                    &exact_targets,
                                    cfg,
                            ...
                            {
                                let polish_steps = steps.min(if headless_fast_profile { 12 } else { 16 }).max(4);
                                ...
                                let candidate_exact_loss = candidate_calibration
                                    ...
                                    .and_then(|candidate_calibration| {
                                        let candidate_exact_samples = exact_targets
                                            .iter()
                                            .map(|sample| {
                                                Self::build_burn_physics_sample(
                                                    sample,
                                ...
                                .unwrap_or(f64::MAX);
                                stage_notes.push(format!(
                                    "PINOExactRefineCandidate: round={}, optimizer={:?}, steps={}, lr={:.6e}, exactLoss={:.6e}, trainLoss={:.6e}, score={:.6e}",
                                    round + 1,
                                    optimizer,
                                    steps,
                                    learning_rate,
                                ...
                                let loss_margin = refinement_best_loss
                                    .abs()
                                ...
                                let holdout_ok = !refinement_best_score.is_finite()
                                    || candidate_score
                                        <= refinement_best_score * holdout_regression_limit + score_margin;
                                let loss_gain =
                                    candidate_exact_loss + loss_margin < refinement_best_loss;
                                let focused_exact_override = (focused_plate_hole
                                    || focused_cantilever)
                                    && loss_gain
                                    && candidate_exact_loss.is_finite()
                                    && (headless_fast_profile || candidate_score.is_finite());
                                if (holdout_ok && loss_gain) || focused_exact_override {
                                    refinement_best_score = refinement_best_score.min(candidate_score);
                                    refinement_best_loss = refinement_best_loss.min(candidate_exact_loss);
                                    refinement_best_params = candidate_params;
                                    refinement_best_lr = learning_rate;
                                    if let Some(candidate_calibration) = candidate_calibration {
                                        refinement_best_calibration = Some(candidate_calibration);
                                    }
                                    refinement_improved = true;
                                    round_improved = true;
                                    refinement_stall_count = 0;
                                    if refinement_best_loss <= routed.target_loss {
                                        break 'refine;
                                    }
                                    if headless_fast_profile && refinement_best_loss <= 5.0e2 {
                                        break 'refine;
                                    }
                                }
                            }
                    ...
                    if refinement_improved {
                        ...
                        best_effective_val = best_effective_val.min(refinement_best_loss);
                        ...
                        if let (Some(meta), Some(calibration)) =
                            (best_runtime_meta.as_mut(), best_runtime_calibration.as_ref())
                        {
                            meta.calibration_stress_scale = Some(calibration.stress_scale);
                            meta.calibration_displacement_scale =
                                Some(calibration.displacement_scale);
            ...
            } else if low_target_mode && skip_exact_refine {
                stage_notes.push(
            ...
            return TrainResult {
                ...
                stop_reason: if forced_stop_reason.is_some() {
...
mod tests {
    use super::*;
    use crate::contracts::{LoadInput, SolveInput};
    ...
    fn pino_runtime_emits_preflight_before_epoch_progress() {
        ...
        assert!(
            events.iter().any(|(epoch, stage, _)| *epoch == 0 && stage == "preflight"),
    ...
    fn pino_runtime_snapshot_resume_preserves_inference_shape() {
        ...
        assert!((before.fem_like.von_mises_psi - after.fem_like.von_mises_psi).abs() <= 1e-6, "resume changed inferred von-mises beyond tolerance: before={} after={}", before.fem_like.von_mises_psi, after.fem_like.von_mises_psi);
        assert_eq!(
    ...
    fn pino_headless_default_target_1e9_diagnostic() {
        ...
        let stage1_epochs = std::env::var("PINO_HEADLESS_STAGE1")
            .ok()
            .and_then(|v| v.parse::<usize>().ok())
            .unwrap_or_else(|| ((max_total_epochs as f64) * 0.08).round().clamp(24.0, 160.0) as usize);
        let stage2_epochs = std::env::var("PINO_HEADLESS_STAGE2")
            .ok()
            .and_then(|v| v.parse::<usize>().ok())
            .unwrap_or_else(|| ((max_total_epochs as f64) * 0.14).round().clamp(40.0, 280.0) as usize);
        let stage3_epochs = std::env::var("PINO_HEADLESS_STAGE3")
            .ok()
            .and_then(|v| v.parse::<usize>().ok())
            .unwrap_or_else(|| ((max_total_epochs as f64) * 0.18).round().clamp(48.0, 360.0) as usize);
        ...
        println!(
            "pino-headless-1e9-config: max_total={} max_backoffs={} stage1={} stage2={} stage3={} collocation={} boundary={} interface={} max_topology={} progress_every={} network_every={} online_active_learning={} max_runtime_s={} skip_exact_refine={}",
            max_total_epochs,
            max_backoffs,
            stage1_epochs,
            stage2_epochs,
